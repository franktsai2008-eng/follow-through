...
Left rail (the form): company names (two inputs, defaults "Northline Procurement" / "Harbor Packaging"), flow presets as segmented pills (agree a price / chase an invoice / chase a delivery / blank — keep the three preset card texts already in the file), your card textarea, "their card" collapsible, recipient email (default from /status or `the owner's One account address`), their agent model select (GPT via Codex / Claude), rounds (2–8, default 6), two toggles (use lessons from earlier cases; ground both cards with a You.com search — default on), Run (white pill, black text). Below: "Recalled" block: the lessons that will be used, numbered, with a count chip; "clear lessons" ghost button. Bottom: "Past runs" list from /runs; clicking loads that job into the main view.
